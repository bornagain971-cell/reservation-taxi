...
(function(){
  ...
  async function getEstimate(){
    // Prefer a global compute if available (sync or async)
    try{
    ...
    // Fallback to dataset/text on page
  ...
  async function maybeHandleMailto(e){
    ...
    if(!a) return;
    e.preventDefault();
    // Compute/refresh estimation right now
    let price = NaN;
  ...
  ['click','pointerdown','touchstart'].forEach(evt => {
    document.addEventListener(evt, maybeHandleMailto, true); // capture
